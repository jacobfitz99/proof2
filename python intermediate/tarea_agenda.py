...
while eleccion != "5":
	...
	if eleccion == "1":
		with open("agenda.txt","a") as agenda: #renombra la variable 
			datos=["\nnombre"," ","numero\n"]
			datos[0]=input("Nombre del contacto: ").upper()
			numero = input("Numero telefonico: ").upper()
			datos[2] = numero+"\n"
			agenda.writelines(datos) #convierte en listas el archivo
			menu()
	elif eleccion == "2":
		modificando=input("¿A quien deseas modificar?\n").upper()
		nuevoNombre=input("Ingresa nuevo nombre:  ").upper()
		nuevoNumero=input("Ingresa nuevo numero:  ").upper()
